...
class WebhookService extends Service {
  async translateMsg(data) {
    let { object_kind, event_name } = data || {};
    if (!object_kind && event_name) {
    ...
    const content = [];
    switch (object_kind) {
      case OBJECT_KIND.repository_update:
        res = await this.assembleRepositoryUpdateMsg(content, data);
        break;
  ...
  async assembleRepositoryUpdateMsg(content, { user_name, changes, project }) {
    const { name: projName, web_url, path_with_namespace } = project || {};

    const op = '仓库更新';
    for (let i = 0; i < changes.length; i++) {
      const branch = changes[i].ref.replace('refs/heads/', '');
  ...
  async assembleIssueMsq(content, { user, project, repository, object_attributes, assignees, assignee, labels }) {
    ...
    const responsible = assignees.length > 0 ? assignees[0].name : '无';
  ...
  formatCommits(commits) {
    ...
    const result = {
      commits: commits.map(commit => {
        ...
        changes.added += added ? added.length : 0;
        changes.modified += modified ? modified.length : 0;
        changes.removed += removed ? removed.length : 0;
